...
* **Parser extended beyond `<i>`/`<br>` (runtime finding).** The "intentionally minimal" policy preserved unknown tags verbatim, which leaked raw markup into the UI — the DB's Reading fields wrap content in `<p class="textitalic">…</p>` (and contain `span`/`sup`/`a`/`table` tags). The parser now treats `<p>` as a paragraph break (collapsing consecutive breaks, trimming edge breaks) and strips unsupported tags while keeping their inner text.
* **Default-route page attaches its VM lazily (runtime finding).** `DailyReflectionPage` is created during `NavigateAsync` in `App.OnLaunched`, before `App.Host` exists, so constructor-time `App.GetService<T>()` threw and the Reflection tab rendered blank on every platform. The page now attaches its VM on first `Loaded` (via `DataContext` when the navigator supplies it, else the service locator) — see `PageBase.OnBeforeActivate`.
