...
* **Migration trigger predicate.** `MigrateSettingsIfNeeded` no longer probes a "legacy" preferences container — there is no such container on Uno. The condition therefore reduces to "first launch of a build ≥ 20 with no previous build/version recorded", which is the exact predicate the Xamarin original applied to upgrades. On a greenfield Uno install the condition is true on the very first launch, so `MigrateOldPreferences()` is invoked (the implementation is a no-op round-trip in that case, matching its current behaviour). Spec 006 §10.6.1 will revisit the migration semantics. **Revisited:** `MigrateOldPreferences()` is no longer a no-op round-trip — with the bundle id flipped to `com.kazo0.dailyreflection` (spec 011 §E), the legacy Xamarin stores survive the in-place upgrade, and the Android/iOS `SettingsService` partials (`ImportLegacyPreferences`) import the five preference keys from Android SharedPreferences / the iOS `DR_Settings` NSUserDefaults suite into `LocalSettings`. On greenfield installs the import finds nothing and remains a no-op.
