...
This branch contains the **Uno Platform port** of the original Xamarin.Forms app ([kazo0/DailyReflection](https://github.com/kazo0/DailyReflection)). The port is designed to upgrade the Xamarin app **in place** on the stores: it keeps the original `ApplicationId` (`com.kazo0.dailyreflection`) and moves to 4.x versions computed by Nerdbank.GitVersioning (packed Android versionCode ≥ 67108864, above the Xamarin app's 3.4 (34)). On first launch after the upgrade, a one-time startup migration imports user settings (sober date, notification time/enabled, display preference) from the legacy platform stores (Android SharedPreferences, iOS `DR_Settings` NSUserDefaults suite) and re-schedules the daily notification.
...
DailyReflection.Services       Service interfaces (ISettingsService, IShareService, IClipboardService, INotificationService,
                               ...
                               StartupMigrationRunner (one-time legacy settings import, version-gated
                               DB refresh, daily-notification re-arm).
...
scripts/wasm-aot-profile             Node + Playwright recorder for the WebAssembly head's AOT profile
                                     (Platforms/WebAssembly/aot.profile); see docs/WEB-DEPLOY.md.
```
...
- **Startup migrations** (`StartupMigrationRunner`) run after first paint and port the Xamarin `App.OnStart` behaviour. The legacy settings import runs once per install, recorded by the `LegacySettingsImported` preference (set only after it succeeds, so a failure retries next launch) — *not* by version thresholds: NBGV versions (`4.0.N`, `4.0.N-alpha-g…`) aren't numbers, and the Xamarin-era gate parsed them as 0, so the import silently never ran. The database refresh keeps its `VersionConstants` gate, read culture-invariantly as major.minor. Every launch also re-arms the daily notification when it is enabled (never prompting for permission, except on the launch that imported legacy settings). Be careful here — this is the store-upgrade path for real users; `StartupMigrationRunnerTests` covers it with the version strings the app actually ships.
- Code comments reference design docs by spec number and section (e.g. `Spec 004 §D`) — see `specs/`.

### Known platform issues (upstream)

- **Android 16+ Back button** — Uno ignores `BackRequestedEventArgs.Handled` on API 36+ while anything subscribes to `SystemNavigationManager.BackRequested`, and every Toolkit `NavigationBar` subscribes for as long as it is loaded, so Back could never leave the app ([unoplatform/uno.toolkit.ui#1651](https://github.com/unoplatform/uno.toolkit.ui/issues/1651)). `MainActivity.RootBackPressedCallback` works around it: it forwards each press through Uno with a probe handler and sends the task to the background when nothing handled it. Open flyouts/pickers still close on Back. Remove the callback once the Toolkit only subscribes while it can navigate back.
- **Skia desktop: stale rendering after a tab switch** — content that changes while its tab is collapsed (e.g. toggling Secular Readings in Settings, then returning to Reflection) can show overlapping text or fragments of the previous tab until the window is resized. Layout is correct (an offscreen render of the same tree is fine); it is Uno's compositor, reproduced on Uno.Sdk 6.7.30 through 7.0-dev with Metal and software rendering, desktop only ([unoplatform/uno#24768](https://github.com/unoplatform/uno/issues/24768)). No app-side workaround found (an opaque region background, `InvalidateMeasure`, and a 1px margin nudge were all tried).
- **Release logging** — console output doesn't reach logcat in the Android Native AOT builds, so Android logs through `Platforms/Android/LogcatLoggerProvider.cs` (tag `DailyReflection`). Release builds log at Warning and up, with `Uno.UI.ResourceResolver` raised to Error (~1600 harmless Material-resource warnings per launch otherwise).
...
# Unit tests (NUnit) — verified green: 39 presentation + 45 services = 84 tests
...
- All 84 unit tests pass on .NET SDK 10.0.401 as of this writing; keep them green.
...
- CI/CD is **GitHub Actions**:
  ...
  - `.github/workflows/release.yml` — triggered by any push to a `release/*` branch: computes/validates the version, runs tests, builds a signed `.aab`/`.apk` and a signed `.ipa` (both **Native AOT** — see "Native AOT publish" above; the jobs pass the `PublishNativeAot` switch and point Android at the runner's `ANDROID_NDK_HOME`, r27.3), and self-contained desktop zips (win-x64 / linux-x64 / osx-arm64), then **waits for manual approval** on the `production` GitHub Environment before uploading + submitting to App Store Connect (fastlane `deliver`, first, since it is the step most likely to be refused), uploading to Google Play, and creating a GitHub release — which pushes the `vX.Y.Z` tag. `workflow_dispatch` inputs allow dry runs (Play test track, skip App Store review submission, `native_aot=false` to ship the mobile packages with the runtime's Mono AOT instead). App Store Connect refuses an update without "What's New": `deliver` uploads it from `fastlane/metadata/default/release_notes.txt` (the only metadata file, so nothing else in the listing changes) — **update that file for every release**; the step fails early if it is empty. It also re-records the WebAssembly AOT profile for the production web build and forward-ports it (see "WebAssembly: profile-guided AOT").
  - **Web (Cloudflare Pages)**: `.github/workflows/web-deploy.yml` is a reusable workflow that publishes `net10.0-browserwasm` (profile-guided AOT, see above) and uploads it to one Pages branch (slot): `pr-<n>` from ci.yml, `staging` from alpha.yml (→ `dailyreflection-staging.kazo0.dev`), and `production` from release.yml's `deploy-web`, which `needs: publish` so it runs only after the `production` approval and never asks for a second one. The deploy job skips itself until the `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` secrets exist, and it creates the Pages project on first run if it's missing. Custom domains live in the Cloudflare dashboard, not the repo. `docs/WEB-DEPLOY.md` has the setup, rollback, and caching notes (`_headers` in `Platforms/WebAssembly/wwwroot`). `.github/workflows/web-preview-cleanup.yml` deletes a PR's `pr-<n>` preview deployments when it closes, plus a daily sweep for stragglers; a manual run defaults to `dry_run`. Its logic is `scripts/pr-preview-cleanup/cleanup.cjs` (tests run in ci.yml's Unit tests job). It runs on `pull_request_target`, so it always checks out the default branch and never executes PR-head code.
