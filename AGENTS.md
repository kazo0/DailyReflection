...
- **WinUI 3 XAML** rendered by the Uno Skia renderer; **Uno Material** theme (`MaterialToolkitTheme` in `App.xaml`, whose whole Light/Dark palette is generated from one seed — `ThemeColors PrimarySeed="#1976D2"` (the Xamarin-era DR blue) with `SeedColorMode="Fidelity"`, which keeps the seed's full chroma; there is no hand-written color override file. Fidelity makes `#1976D2` the primary *container* and puts `PrimaryColor` at a darker tone of the same blue, so to pin an exact role add `OverrideSource` to that `ThemeColors`, which beats the generated values); **Uno.Toolkit `TabBar`** for the tab chrome — bottom bar on narrow windows, vertical rail (`VerticalTabBarStyle`) on wide ones, switched by the Toolkit's `{utu:Responsive}` markup extension — and **Uno.Toolkit `NavigationBar`** as the top app bar on every page.
