...
- **Preview cleanup:** `web-preview-cleanup.yml` deletes deployments for `pr-<number>` when that PR closes (merged or unmerged). A daily sweep at 06:23 UTC also removes older closed-PR previews and catches uploads that finished after the close-event cleanup. Only deployments whose environment is `preview` and whose branch exactly matches `pr-<number>` qualify; staging, production, unrelated branches and open/reopened PRs are preserved. It checks the current GitHub PR state before each delete and collects all deployment pages before deleting. API errors fail the run rather than being treated as successful cleanup. The Cloudflare API's `force=true` option allows deleting aliased non-production deployments, including the PR branch alias.
- **Running cleanup:** Once the workflow is on `master`, use Actions → Clean up web previews → Run workflow. `dry_run` defaults to true and logs the deployments it would delete; uncheck it to delete them. Automatic close-event and scheduled runs delete without a dry run. It reuses the deploy workflow's Cloudflare token/account/project settings and skips if credentials are absent. The privileged close-event workflow always checks out the default branch, never PR-head code. No live cleanup occurs merely by adding this workflow in a PR. Run its dependency-free tests locally with `node --test scripts/pr-preview-cleanup/cleanup.test.cjs`; the CI Unit tests job runs them too.
